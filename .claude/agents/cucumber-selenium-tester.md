...
- Driver setup is a single, consistent pattern: `MyStepDefinitions` launches headless `ChromeDriver`. No driver binary is checked into the repo — Selenium Manager (bundled in `selenium-java`) auto-detects the installed browser and downloads a matching `chromedriver`, so don't reintroduce a checked-in binary or a hardcoded driver path. `CucumberseleniumApplication.main` is a plain Spring Boot bootstrap with no Selenium code — leave it that way.
  - `pom.xml` pins `<selenium.version>` as a property (not just `selenium-java`'s `<version>`) because `spring-boot-starter-parent:3.1.5` otherwise silently downgrades Selenium's submodules to 4.8.3 via its own dependency management. If you bump Selenium, bump that property too.
  - Only set the `chrome.binary.path` system property when running in a sandbox where Chrome/Chromium isn't on a standard system path — never hardcode it in source.
