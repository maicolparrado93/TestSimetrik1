...
**The test step runs with `continue-on-error: true`, deliberately.** Google shows a reCAPTCHA to GitHub Actions' shared runner IPs instead of real search results — confirmed by inspecting the page HTML in a CI run (`<div id="recaptcha" class="g-recaptcha">` appears instead of `#result-stats`/organic results). It is not a selector bug or a flake, and there is no code fix for it (working around Google's bot-detection is out of scope, deliberately). The `Report suite outcome` step writes the real pass/fail to the run's Job Summary regardless, so the failure stays visible without turning the PR check red.

Only `@BuscarPalabra` and `@BuscarSelenium` fail deterministically in CI — both go through `obtenerTotalResultados()`, which needs `#result-stats` or organic result links, neither of which render behind the reCAPTCHA. The other five scenarios pass reliably in CI: `@CampoBusquedaVisible`, `@TituloPaginaInicial` and `@CampoBusquedaEditable` never trigger a search at all, and `@BuscarTituloCucumber`/`@BuscarFraseCompuesta` pass because Google still reflects the query in the page `<title>` even while showing the reCAPTCHA for the results themselves — so don't assume every search-triggering scenario is blocked; only the result-count-based assertion is. All seven scenarios pass locally from a normal (non-datacenter) IP.
